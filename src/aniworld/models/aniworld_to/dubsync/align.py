...
def resample_dub(dub_path, dest_path, tempo_ratio: float, label: str = "") -> Path:
    ...
    result is re-encoded (FLAC, so no further generational loss after the
    decode). Only called when the user opted in via allow-resample.
    """
